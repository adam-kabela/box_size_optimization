...
def can_be_packed(cards, box):
    # for instance, use rectpack
    # from rectpack import newPacker...
    # we apply a more efficient approach with best_boxes_for_this_order()
    pass

# Get all best boxes for each order, best means that neither width nor length of the box can shrink.
# Note that one order can have more best (incomparable) boxes.
...
def best_containers_for_this_order(cards):
        ...
        for rotation in [0,90]:
            ...
            for p in possible_positions(grid_lines):
                if overlaps_ok(layout, r, p):
                    ...
                    possible_containeres = possible_containeres + backtracking_recurrence(new_layout, new_remaining_rectangles, new_grid_lines)
    return keep_minimal(possible_containeres)

def keep_minimal(possible_containeres):
    best_containeres = []
    possible_containeres.sort(key = lambda x: (x[0], x[1])) #sort by width and if tie use length
    for p in possible_containeres: #sorted from smallest
        is_best = True    
        for b in best_containeres:
            if b[0] <= p[0] and b[1] <= p[1]:
                is_best = False
                break
        if is_best:
            best_containeres.append(p)
    return best_containeres
